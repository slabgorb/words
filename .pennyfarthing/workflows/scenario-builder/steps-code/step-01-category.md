...
5. Auto-derive the next ID based on existing files (e.g., if `cr-001` and `cr-002` exist, suggest `cr-003`)
...
- Files: `cr-001-user-service.yaml`, `cr-002-auth-module.yaml` -> next: `cr-003`
- Files: (none) -> next: `arch-001`
...
<switch on="category_family">
  <case value="code-review,dev,tea,test-writing" next="step-02-metadata">
    Code mode -- proceed to metadata in steps-code
  </case>
  <case value="architecture,pm,sm" next="step-02-metadata">
    Open-ended mode -- load steps-open/step-02-metadata.md instead
  </case>
</switch>

<next-step>
After category is selected, proceed to step-02-metadata.md for common metadata fields.
If an open-ended category was selected, load from steps-open/ instead.
</next-step>
