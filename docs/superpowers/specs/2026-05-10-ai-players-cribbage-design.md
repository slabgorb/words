...
glyph: ♡
systemPrompt: |
  You are Granny, who has been playing cribbage at the Methodist church
...
Loaded once at server boot into an in-memory catalog. v1 ships with 2-3 personas (e.g., Granny, the Hustler, the Mathematician).
...
| Failure | Behavior |
|---|---|
| Claude returns invalid JSON | Retry once with terser system reminder. If still bad → first legal move, log warning, no banter. |
| Claude picks illegal move | Same as above. |
| Subprocess timeout (default 30s) | First legal move, log warning, no banter. |
| `claude` CLI not installed | Fail-loud at server boot if any bot users exist (don't silently disable). |
| `ai_sessions` row missing for a bot's turn | Recreate it (defensive); log warning. |
| Server restart mid-game | Session resumed from persisted `claude_session_id`; if Claude rejects the resume, create a new session and prepend a "you are mid-game, here is the current state" recap. |
| Subscription has no listeners | Bot still plays; banter just doesn't reach anyone. |

No silent fallback to fake AI. If Claude breaks, the bot still moves so the game progresses, but it's logged loudly.
...
- `src/server/routes.js` — accept `personaId` in `POST /api/games`; create `ai_sessions` row when opponent is a bot.
- `src/server/migrate.js` — register the new migration.
- `src/server/server.js` — boot AI subsystem (seed bot user, load personas).
...
- **Action queueing across server restarts.** If the server restarts while a bot's turn is enqueued, the next human action will trigger the bot turn naturally. But if the bot is the *only* active player (e.g., long pegging streak), nothing wakes it. Mitigation: on boot, scan `games` where `activeUserId` is a bot and `status='active'`; enqueue them.
...
- **Token budget per turn.** Default subprocess timeout 30s is conservative. If Claude is slow on a long-context session, the cribbage feel may suffer. Monitor in practice; revisit if needed.
